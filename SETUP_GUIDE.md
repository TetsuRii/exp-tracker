...
2. **Register a new account** (any username + password). The old app's data was on a
   temporary hard drive that Render wipes — it's gone. New accounts save to MongoDB
   permanently and work on every device.
3. Add products → the suggestion dropdown now searches ALL 14,000+ products.
4. To import your Excel CSV: click the 🤖 button → **📥 Import Products** → choose the CSV file.
