  ...
  req.on('end', async () => {
    ...
    try {
      ...
      if (p === '/api/register' && req.method === 'POST') {
        ...
        s.bind([username]); if (s.step()) { s.free(); sendJSON(res, 400, { error: 'Username taken' }); return; } s.free();
        const hash = bcrypt.hashSync(password, 10);
        db.run('INSERT INTO users (username, password) VALUES (?, ?)', [username, hash]); saveDb();
        ...
        sendJSON(res, 200, { token, user: { id: uid, username } }); return;
      }
      if (p === '/api/login' && req.method === 'POST') {
        const { username, password } = parsed;
        const s = db.prepare("SELECT id, username, password FROM users WHERE username = ?");
        s.bind([username]); if (!s.step()) { s.free(); sendJSON(res, 401, { error: 'Invalid credentials' }); return; }
        ...
        sendJSON(res, 200, { token, user: { id: u.id, username: u.username } }); return;
      }
      ...
      if (p.startsWith('/api/products/') && req.method === 'PUT') { if (!auth()) return;
        const id = p.split('/')[3]; const d = parsed; const st = d.stores||{};
      ...
      if (p === '/api/history' && req.method === 'GET') { if (!auth()) return;
        ...
        s.bind([userId]); const rows = []; while (s.step()) rows.push(s.getAsObject()); s.free();
        sendJSON(res, 200, rows.map(r => ({ id: r.id, productId: r.product_id, productName: r.product_name, category: r.category||'', date: r.date, dateRecorded: r.date_recorded||'', expiryDate: r.expiry_date||'', quantity: r.quantity, unit: r.unit, price: r.price||0, notes: r.notes||'', skip: !!r.skip, createdAt: r.created_at, stores: { lowthers: r.store_lowthers||0, valley: r.store_valley||0, laTante: r.store_la_tante||0 } }))); return;
      }
      ...
      if (p === '/api/reference' && req.method === 'POST') { if (!auth()) return;
        ...
        s.free(); saveDb(); sendJSON(res, 200, { success: true, count: products.length }); return;
      }
    ...
    } catch (err) { console.error('Server error:', err.message); sendJSON(res, 500, { error: err.message }); }
  });
});
