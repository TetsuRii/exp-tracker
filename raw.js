...
<p style="color:#94a3b8">The server retries the connection every 30 seconds — no redeploy needed once fixed.</p>
      ...
      if (pathname === '/api/products' && req.method === 'POST') { if (!a()) return;
        ...
        // Prevent duplicates: if a product with the same name and expiry date exists,
        // merge the incoming values into it (keep the higher quantity/store counts).
        if (nm) {
          ...
          if (dup) {
            const qty = Math.max(dup.quantity||1, d.quantity||1);
            const l = Math.max(dup.storeLowthers||0, st.lowthers||0);
            const v = Math.max(dup.storeValley||0, st.valley||0);
            const t = Math.max(dup.storeLaTante||0, st.laTante||0);
            await productsCol.updateOne({ id: dup.id, userId }, { $set: {
              quantity: qty, storeLowthers: l, storeValley: v, storeLaTante: t,
      ...
      if (pathname === '/api/products/dedupe' && req.method === 'POST') { if (!a()) return;
        try {
          ...
          // Group by same name (case-insensitive) + same expiry date
          const groups = new Map();
          for (const p of all) {
            const key = ((p.name||'').trim().toLowerCase() + '|' + (p.expiryDate||'')).trim();
          ...
          for (const list of groups.values()) {
            ...
            // Keep the most complete record (has barcode/notes, otherwise earliest)
            list.sort((a, b) => {
              const score = p => (p.barcode ? 2 : 0) + (p.notes ? 1 : 0);
            ...
            let qty = keeper.quantity||1;
            let l = keeper.storeLowthers||0, v = keeper.storeValley||0, t = keeper.storeLaTante||0;
            const delIds = [];
            for (const o of others) {
              delIds.push(o.id);
              qty = Math.max(qty, o.quantity||1);
              l = Math.max(l, o.storeLowthers||0);
              v = Math.max(v, o.storeValley||0);
              t = Math.max(t, o.storeLaTante||0);
              if (!keeper.barcode && o.barcode) keeper.barcode = o.barcode;
              if (!keeper.notes && o.notes) keeper.notes = o.notes;
              if ((!keeper.category || keeper.category === 'Other') && o.category && o.category !== 'Other') keeper.category = o.category;
            }
            await productsCol.updateOne({ id: keeper.id, userId }, { $set: {
              quantity: qty, storeLowthers: l, storeValley: v, storeLaTante: t,
              barcode: keeper.barcode||'', notes: keeper.notes||'', category: keeper.category||'Other',
              updatedAt: new Date().toISOString()
            }});
            if (delIds.length) {
              // Point history entries at the kept product so nothing gets lost
              await historyCol.updateMany({ userId, productId: { $in: delIds } }, { $set: { productId: keeper.id } });
              await productsCol.deleteMany({ userId, id: { $in: delIds } });
            }
            groupsMerged++; removed += delIds.length;
          }
          const remaining = await productsCol.countDocuments({ userId });
          sj(res, 200, { success: true, groupsMerged, duplicatesRemoved: removed, remainingProducts: remaining });
