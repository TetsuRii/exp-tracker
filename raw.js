...
http.createServer((req, res) => {
  ...
  req.on('end', async () => {
    ...
    try {
      ...
      // AI: Build reference list from all_products.json + user's products
      if (pathname === '/api/ai/build-ref' && req.method === 'POST') { if (!a()) return;
        try {
          ...
          // 1. Load from all_products.json
          const jsonPath = path.join(__dirname, 'all_products.json');
          let jsonItems = [];
          try { if (fs.existsSync(jsonPath)) jsonItems = JSON.parse(fs.readFileSync(jsonPath, 'utf8')); } catch(e) {}
          if (!jsonItems.length) {
            const parentPath = path.join(__dirname, '..', 'all_products.json');
            try { if (fs.existsSync(parentPath)) jsonItems = JSON.parse(fs.readFileSync(parentPath, 'utf8')); } catch(e) {}
          }
            ...
            for (const item of jsonItems) {
              if (item.name && !seenNames.has(item.name)) {
                ...
                toInsert.push({
                  ...
                  userId,
                  name: item.name,
                  category: item.category || guessCategory(item.name).category || 'Other',
                  barcode: item.barcode || ''
                });
              }
            }
            for (let i = 0; i < toInsert.length; i += 500) {
              await refProductsCol.insertMany(toInsert.slice(i, i + 500));
            }
            added = toInsert.length;
          }

          // 2. Also copy from user's products
          ...
          const finalCount = await refProductsCol.countDocuments({ userId });
          sj(res, 200, { success: true, added: added + addedFromProducts, totalRef: finalCount });
        } catch(e) { sj(res, 500, { error: e.message }); }
        return;
      }
...
async function suggestProductsMongo(userId, partial, limit = 50) {
  ...
  try {
    ...
    // 1. Search reference_products (imported product list)
    ...
    // 3. If typing a number, search by barcode too
