...
<p style="color:#94a3b8">The server retries the connection every 30 seconds — no redeploy needed once fixed.</p>
      ...
      if (pathname === '/api/products' && req.method === 'POST') { if (!a()) return;
        ...
        // Prevent EXACT duplicates: only merge when name, expiry date, quantity,
        // unit, date recorded AND all store amounts are identical. If anything
        // differs (e.g. different quantity), it's a separate row and stays.
        if (nm) {
          const dup = await productsCol.findOne({
            userId,
            name: { $regex: '^' + escapeRegex(nm) + '$', $options: 'i' },
            expiryDate: d.expiryDate||'',
            quantity: d.quantity||1,
            unit: d.unit||'pcs',
            dateRecorded: d.dateRecorded||'',
            storeLowthers: st.lowthers||0,
            storeValley: st.valley||0,
            storeLaTante: st.laTante||0
          });
          if (dup) {
            // Exact duplicate: keep the existing row (fill in barcode/category if the new one has them)
            await productsCol.updateOne({ id: dup.id, userId }, { $set: {
              barcode: dup.barcode || d.barcode || '',
              category: (dup.category && dup.category !== 'Other') ? dup.category : (d.category || 'Other'),
              updatedAt: new Date().toISOString()
            }});
            sj(res, 200, { success: true, id: dup.id, merged: true }); return;
          }
        }
      ...
      // ONLY collapses rows that are IDENTICAL in name, expiry date, quantity,
      // unit, date recorded, and all store amounts. Products that differ in
      // quantity or stores are NOT duplicates and are left untouched.
      // A full snapshot is saved to 'dedupe_backups' before anything is removed,
      // and /api/products/dedupe/restore can bring it back.
      
      function exactProductKey(p) {
        return [
      ...
      if (pathname === '/api/products/dedupe' && req.method === 'POST') { if (!a()) return;
        try {
          const all = await productsCol.find({ userId }).toArray();
          
          // Safety: refuse to run if any row is missing its id (can't delete safely)
          const missingId = all.filter(p => !p.id);
          if (missingId.length) {
            sj(res, 400, { error: 'Cannot run: ' + missingId.length + ' product(s) are missing an id. Contact support.' });
            return;
          }
          
          // Snapshot BEFORE any changes (safety net)
          const snapshotId = new Date().toISOString();
          await db.collection('dedupe_backups').insertOne({
            snapshotId, reason: 'dedupe-run',
            createdAt: new Date(),
            products: JSON.parse(JSON.stringify(all))
          });
          
          // Group by the FULL row (name+expiry+quantity+unit+date+stores)
          const groups = new Map();
          for (const p of all) {
            const key = exactProductKey(p);
            if (!groups.has(key)) groups.set(key, []);
            groups.get(key).push(p);
          }
          
          let groupsMerged = 0, removed = 0;
          const delIds = [];
          const keptById = new Map();
          for (const list of groups.values()) {
            if (list.length < 2) continue;
            // Keep the most complete row (barcode/notes/category), then the earliest added
            list.sort((a, b) => {
              const score = p => (p.barcode ? 2 : 0) + (p.notes ? 1 : 0) + ((p.category && p.category !== 'Other') ? 1 : 0);
              const d = score(b) - score(a);
              if (d) return d;
              return (a.addedAt || '').localeCompare(b.addedAt || '');
            });
            const keeper = list[0];
            keptById.set(keeper.id, keeper);
            for (const o of list.slice(1)) delIds.push(o.id);
            groupsMerged++; removed += list.length - 1;
          }
          
          if (delIds.length) {
            // Repoint history: for each deleted id, find which kept row it matched
            const delToKeeper = new Map();
            for (const list of groups.values()) {
              if (list.length < 2) continue;
              const keeper = list[0];
              for (const o of list.slice(1)) delToKeeper.set(o.id, keeper.id);
            }
            for (const [oldId, newId] of delToKeeper) {
              await historyCol.updateMany({ userId, productId: oldId }, { $set: { productId: newId } });
            }
            await productsCol.deleteMany({ userId, id: { $in: delIds } });
          }
          
          const remaining = await productsCol.countDocuments({ userId });
          sj(res, 200, { success: true, groupsMerged, duplicatesRemoved: removed, remainingProducts: remaining, snapshotId });
        } catch(e) { sj(res, 500, { error: e.message }); }
        return;
      }
      
      // Restore the most recent dedupe snapshot (undo the last Remove Duplicates run)
      if (pathname === '/api/products/dedupe/restore' && req.method === 'POST') { if (!a()) return;
        try {
          const snap = await db.collection('dedupe_backups').find({ reason: 'dedupe-run' }).sort({ createdAt: -1 }).limit(1).next();
          if (!snap || !Array.isArray(snap.products)) { sj(res, 404, { error: 'No dedupe snapshot to restore' }); return; }
          const rows = snap.products.map(p => ({ ...p, userId }));
          await productsCol.deleteMany({ userId });
          if (rows.length) await productsCol.insertMany(rows);
          const count = await productsCol.countDocuments({ userId });
          sj(res, 200, { success: true, restored: count, snapshotId: snap.snapshotId });
        } catch(e) { sj(res, 500, { error: e.message }); }
        return;
      }
