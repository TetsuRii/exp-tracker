...
function search(){
  ...
  w.innerHTML='<input type="text" id="ai_si" placeholder="🔍 Search products by name, category..." autocomplete="off"><div class="ct" id="ai_sc"></div>'
...
function searchTab(){
  ...
  if(!w)return
  // Hide if a dialog/modal is open
  ...
  // Check if we're on the tracker tab
  var tabs=document.querySelectorAll('[class*="tab"].active,[aria-selected="true"],button.active')
  var onTracker=false
  for(var i=0;i<tabs.length;i++){
    var t=tabs[i].textContent.toLowerCase()
    if(t.indexOf('track')>-1||t.indexOf('product')>-1||t.indexOf('inventory')>-1||t.indexOf('all')>-1){onTracker=true;break}
  }
  // Also show if there's a table with data
  var tables=document.querySelectorAll('table')
  var hasData=false
  for(var i=0;i<tables.length;i++){
    if(tables[i].offsetWidth>0&&tables[i].querySelectorAll('td').length>1){hasData=true;break}
  }
  if(hasData||onTracker){
...
function suggest(){
  ...
  // Also find ANY unenhanced input on the page
