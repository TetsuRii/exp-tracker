...
function dedupe(){
  ...
  p.style.cssText='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#fff;border-radius:16px;padding:20px;z-index:999999;box-shadow:0 10px 60px rgba(0,0,0,0.3);width:380px;max-width:92vw;text-align:center'
  p.innerHTML='<div style="font-size:34px">🧹</div>'+
    ...
    '<p style="font-size:13px;color:#666;margin:0 0 14px;line-height:1.5">Removes only <b>exact duplicates</b> — same name, expiry date, quantity, and store amounts. Products with different quantities or store counts are <b>kept</b>. A backup is saved first so you can undo.</p>'+
    ...
    '</div>'+
    '<button id="_dp_restore" style="width:100%;padding:8px;border-radius:8px;border:none;background:#fff7ed;color:#c2410c;font-size:12px;font-weight:600;cursor:pointer">↩️ Undo last Remove Duplicates (restore)</button>'
  document.body.appendChild(p)
  document.getElementById('_dp_cx').onclick=function(){p.remove()}
  document.getElementById('_dp_restore').onclick=async function(){
    var btn=document.getElementById('_dp_restore');btn.textContent='Restoring...';btn.disabled=true
    try{
      var r=await fetch('/api/products/dedupe/restore',{method:'POST',headers:{'Authorization':'Bearer '+t}})
      var d=await r.json().catch(function(){return{}})
      p.remove()
      if(!r.ok){toast('⚠️ '+(d.error||'Failed'),1);return}
      toast('↩️ Restored '+d.restored+' products from backup')
  ...
  document.getElementById('_dp_go').onclick=async function(){
    ...
    try{
      ...
      p.remove()
      if(!r.ok){toast('⚠️ '+(d.error||'Failed'),1);return}
      if(d.duplicatesRemoved===0){toast('✅ No exact duplicates found — '+d.remainingProducts+' products')}
      else{toast('🧹 Removed '+d.duplicatesRemoved+' exact duplicate(s) — '+d.remainingProducts+' products remain')}
