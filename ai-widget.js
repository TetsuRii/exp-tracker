      ...
      if(url.indexOf('/api/products')>-1&&(!args[1]||args[1].method==='GET'||!args[1].method)){
        resp.clone().json().then(function(prods){
          if(Array.isArray(prods)&&prods.length){
            ...
            _renderTimer=setTimeout(function(){renderDates(prods)},2000)
          }
        }).catch(function(){})
      }
...
function renderDates(prods){
  // FIRST: Remove ALL existing date badges to prevent stacking
...
async function buildRef(){var t=token();if(!t){toast('Log in',1);return}bm('📋 Building suggestion list...');tp()
  ...
  if(!r.ok){var ed=await r.json().catch(function(){});bm('⚠️ '+(ed?.error||'Failed'));return}var d=await r.json();bm('✅ Built! Added '+d.added+' new products, total '+d.totalRef+' in suggestion list.')}catch(e){ntp();bm('⚠️ Network error.')}}

function importProds(){
  var t=token();if(!t){toast('Log in',1);return}
  // Create a textarea popup for pasting products
  var existing=document.getElementById('_import_popup')
  if(existing){existing.remove();return}
  var popup=document.createElement('div');popup.id='_import_popup'
  popup.style.cssText='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#fff;border-radius:16px;padding:20px;z-index:999999;box-shadow:0 10px 60px rgba(0,0,0,0.3);width:400px;max-width:90vw'
  popup.innerHTML='<h3 style="margin:0 0 10px;font-size:16px;font-weight:600">📥 Import Products</h3>'+
    '<p style="font-size:12px;color:#888;margin-bottom:8px">Paste product names (one per line) or JSON array:</p>'+
    '<textarea id="_import_text" style="width:100%;height:200px;border:2px solid #ddd;border-radius:8px;padding:8px;font-size:13px;resize:vertical;outline:none;box-sizing:border-box" placeholder="Product Name 1&#10;Product Name 2&#10;Product Name 3"></textarea>'+
    '<div style="display:flex;gap:8px;margin-top:10px">'+
    '<button class="bu" id="_import_go" style="flex:1;padding:10px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;background:#00897b;color:#fff;border:none">Import</button>'+
    '<button id="_import_cancel" style="flex:1;padding:10px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;background:#eee;color:#666;border:none">Cancel</button></div>'
  document.body.appendChild(popup)
  document.getElementById('_import_cancel').onclick=function(){popup.remove()}
  document.getElementById('_import_go').onclick=async function(){
    var text=document.getElementById('_import_text').value.trim()
    if(!text){toast('Paste some product names first',1);return}
    var products=[]
    // Try JSON first, then fall back to line-by-line
    try{var parsed=JSON.parse(text);if(Array.isArray(parsed))products=parsed}catch(e){}
    if(!products.length){
      var lines=text.split('\n').filter(function(l){return l.trim().length>0})
      for(var i=0;i<lines.length;i++){
        products.push({name:lines[i].trim(),category:'Other',barcode:''})
      }
    }
    if(!products.length){toast('No valid products found',1);return}
    ...
    try{
      ...
      ntp()
      if(!r.ok){var ed=await r.json().catch(function(){});bm('⚠️ '+(ed?.error||'Import failed'));return}
