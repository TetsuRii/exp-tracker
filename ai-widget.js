...
function boot(){
  ...
  insights()
  suggestWatch()
  // Watch for table changes and re-inject dates, but DEBOUNCED
  var _debounceTimer
  var obs=new MutationObserver(function(){
    clearTimeout(_debounceTimer)
    _debounceTimer=setTimeout(applyDates,600) // Wait 600ms for React to finish
  })
  obs.observe(document.body,{childList:true,subtree:true})
  // Also fetch products every 5 seconds and re-apply
  setInterval(function(){fetchProducts()},5000)
  setInterval(suggestWatch,800)
  // Watch for modal close (user just saved or cancelled) - trigger immediate re-fetch
  setInterval(function(){
    var dls=document.querySelectorAll('[role="dialog"],[aria-modal="true"]')
    var open=false
    for(var i=0;i<dls.length;i++){if(dls[i].offsetWidth>0){open=true;break}}
    if(!open&&window.__modalWasOpen){fetchProducts()}
    window.__modalWasOpen=open
  },1000)
...
function css(){
  ...
  s.textContent=`
.ai_t{position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#1a1a2e;border:2px solid #4caf50;border-radius:12px;padding:12px 20px;z-index:999999;max-width:500px;width:90%;animation:af .3s;color:#fff;font-size:14px;display:flex;align-items:center;gap:12px}
...
#_sb{bottom:160px;right:20px;background:linear-gradient(135deg,#4caf50,#2e7d32)}
#_sb.on{background:linear-gradient(135deg,#ff5252,#d32f2f)}
#_cb{bottom:20px;right:20px;width:56px;height:56px;font-size:24px;background:linear-gradient(135deg,#667eea,#764ba2)}
#_ib{bottom:90px;right:20px;background:linear-gradient(135deg,#f093fb,#f5576c)}
#_ib.bd::after{content:'';position:absolute;top:-2px;right:-2px;width:12px;height:12px;background:#ff4444;border-radius:50%;border:2px solid #fff}
...
#_sbar button.cm{background:#7c4dff}
#_sbar button.x{background:transparent;color:#999;font-size:20px}
...
/* Date badge - use !important to survive React re-renders */
._db{font-size:11px!important;color:#667eea!important;display:block!important;font-weight:600!important;padding:3px 8px!important;background:#f0f2ff!important;border-radius:4px!important;text-align:center!important;margin:3px 0 2px 0!important}
...
#_ip.ap{max-height:400px;height:auto}#_ip .hd{background:linear-gradient(135deg,#f093fb,#f5576c)}
._ic{padding:12px 16px;overflow-y:auto;flex:1}
._ii{background:#fff;border-radius:10px;padding:10px 14px;margin-bottom:8px;display:flex;align-items:flex-start;gap:10px;box-shadow:0 1px 4px rgba(0,0,0,0.06);border-left:3px solid #667eea;font-size:13px;color:#555}
._ii.d{border-left-color:#ff4444}._ii.w{border-left-color:#ffa726}._ii.i{border-left-color:#42a5f5}._ii.s{border-left-color:#66bb6a}
._ii b{color:#333}._ii .ic{font-size:20px;flex-shrink:0}
...
function searchBar(){
  ...
  w.innerHTML='<input type="text" id="_si" placeholder="🔍 Search products by name, category, barcode..." autocomplete="off"><div class="ct" id="_sc"></div>'
  document.body.insertBefore(w,document.body.firstChild)
  document.getElementById('_si').addEventListener('input',function(){
    clearTimeout(window.__s)
    window.__s=setTimeout(doSearch,200)
  })
  setTimeout(function(){document.getElementById('_sr')?.classList.add('v')},500)
  // Hide/show based on modals
  setInterval(function(){
...
function doSearch(){
  ...
  window.__h=setTimeout(function(){document.querySelectorAll('.shi').forEach(function(e){e.classList.remove('shi')})},2000)
}

// ==================== DATES - Fetch from API & Inject ====================
...
function applyDates(){
  ...
  for(var i=0;i<rows.length;i++){
    ...
    // Smart matching: find product by checking if its name appears in the row text
    // To be fast, only check first 200 products (enough for any visible page)
    var matchedProd=null
    var maxCheck=Math.min(_productsCache.length,200)

    for(var p=0;p<maxCheck;p++){
      var prod=_productsCache[p]
      if(!prod||!prod.name)continue
      var pName=prod.name.toLowerCase().trim()

      // Match if product name is in the row text
      // Use indexOf which is very fast
      if(txt.indexOf(pName)>-1){
        // Make sure it's a reasonable match (name is at least 3 chars, not matching EVERY row)
        if(pName.length>=3){
          matchedProd=prod
          break
        }
    ...
    if(cells.length>1){
      ...
      // REMOVE existing badge first then re-add (handles re-renders)
      var old=lastCell.querySelector('._db')
      if(old)old.remove()

      // Create fresh badge
      var span=document.createElement('span')
      span.className='_db'
      if(matchedProd.updatedAt){
        span.textContent='✏️ Last edited: '+ago(matchedProd.updatedAt)
      }else if(matchedProd.addedAt){
        span.textContent='➕ Added: '+ago(matchedProd.addedAt)
      }else{
        span.textContent='📅 No date info'
      }
      // Insert into the last cell
      if(lastCell.children.length>0){
        var br=document.createElement('br')
        lastCell.appendChild(br)
      }
...
function barcode(){
  var b=document.createElement('button');b.id='_sb';b.className='afb';b.textContent='📷'
  ...
  var lt=0
  inp.addEventListener('input',function(){var n=Date.now();if(n-lt<80&&inp.value.length>5){clearTimeout(window.__st);window.__st=setTimeout(function(){scan(inp.value.trim())},100)};lt=n})
...
function tb(){
  var bar=document.getElementById('_sbar'),b=document.getElementById('_sb')
...
function cam(){
  ...
  ov.innerHTML='<div style="position:absolute;border:2px dashed rgba(76,175,80,0.6);border-radius:12px;width:250px;height:120px;pointer-events:none"></div><video id="_cv" autoplay playsinline></video><button class="cx">✕</button>'
...
function chat(){var b=document.createElement('button');b.id='_cb';b.className='afb';b.textContent='🤖';b.onclick=tc;document.body.appendChild(b)}
function insights(){var b=document.createElement('button');b.id='_ib';b.className='afb';b.textContent='📊';b.onclick=ti;document.body.appendChild(b)
  setTimeout(function(){
    ...
    fetch('/api/ai/insights',{headers:{'Authorization':'Bearer '+t}}).then(function(r){return r.json()}).then(function(d){
      if(d.insights&&d.insights.filter(function(i){return i.type==='danger'||i.type==='warning'}).length>0)document.getElementById('_ib')?.classList.add('bd')
    }).catch(function(){})
  },3000)
}
function tc(){if(ins){var ip=document.getElementById('_ip');if(ip)ip.classList.remove('on');ins=false}var p=document.getElementById('_cp');if(!p){mc();return}cht=!cht;p.classList.toggle('on',cht)}
function ti(){if(cht){var cp=document.getElementById('_cp');if(cp)cp.classList.remove('on');cht=false}var p=document.getElementById('_ip');if(!p){mi();return}ins=!ins;p.classList.toggle('on',ins);if(ins&&p.querySelector('._ic').children.length<=1)li(p)}
...
function mi(){ins=true;var p=document.createElement('div');p.className='ap on';p.id='_ip';p.innerHTML='<div class="hd"><h3>📊 Insights</h3><button onclick="this.closest(\'.ap\').classList.remove(\'on\');ins=false">✕</button></div><div class="_ic"><div class="_ie">Loading...</div></div>';document.body.appendChild(p);li(p)}
function li(p){var t=token();if(!t){p.querySelector('._ic').innerHTML='<div class="_ie">⚠️ Log in.</div>';return}
  fetch('/api/ai/insights',{headers:{'Authorization':'Bearer '+t}}).then(function(r){return r.json()}).then(function(d){var c=p.querySelector('._ic');if(!d.insights||!d.insights.length){c.innerHTML='<div class="_ie">📝 Add products first!</div>';return}c.innerHTML=d.insights.map(function(i){return '<div class="_ii '+(i.type==='danger'?'d':i.type==='warning'?'w':i.type==='info'?'i':'s')+'"><span class="ic">'+(i.icon||'📌')+'</span><div><b>'+i.message+'</b>'+(i.items?'<br><sm>'+i.items.join(', ')+'</sm>':'')+(i.count!==undefined?'<br><sm>Count: '+i.count+'</sm>':'')+'</div></div>'}).join('')}).catch(function(){p.querySelector('._ic').innerHTML='<div class="_ie">⚠️ Error.</div>'})}
