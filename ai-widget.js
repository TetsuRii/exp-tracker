...
(function() {
  ...
  function init() {
    ...
    addProductSearchBar();      // 🔍 Product search bar
    watchForProductForms();     // 🧠 Auto-categorize
    watchForNameInputs();
    console.log('🤖 AI Assistant + Barcode Scanner + Search loaded');
  }
  ...
  function addProductSearchBar() {
    // Inject search bar into the page
    const wrap = document.createElement('div');
    wrap.id = 'ai-product-search-wrap';
    wrap.innerHTML = '<input type="text" id="ai-product-search-input" placeholder="🔍 Search products by name, category, barcode, or expiry..." autocomplete="off" />';
    const count = document.createElement('div');
    count.className = 'search-count';
    count.id = 'ai-search-count';
    wrap.appendChild(count);
    document.body.appendChild(wrap);

    const input = document.getElementById('ai-product-search-input');

    // Watch for tab changes to show/hide search bar
    setInterval(checkForTrackerTab, 1000);
    
    // Real-time search
    input.addEventListener('input', function() {
      clearTimeout(searchDebounce);
      searchDebounce = setTimeout(doProductSearch, 200);
    });
  }

  function checkForTrackerTab() {
    const wrap = document.getElementById('ai-product-search-wrap');
    const input = document.getElementById('ai-product-search-input');
    if (!wrap || !input) return;

    // 🛑 HIDE when a modal/dialog/popup is open (user is adding/editing a product)
    const anyModalOpen = document.querySelectorAll(
      '[role="dialog"], .modal-backdrop, .MuiDialog-root, [class*="modal"][style*="display: block"], ' +
      '[class*="modal"][style*="display:block], [class*="overlay"][style*="display: block"], ' +
      '[class*="overlay"][style*="display:block]'
    ).length > 0;
    
    // Also check for dialogs that might not use standard attributes
    const visibleModals = [];
    document.querySelectorAll('[role="dialog"], .modal, [class*="dialog"], [class*="overlay"]').forEach(function(m) {
      var style = window.getComputedStyle(m);
      if (style.display !== 'none' && style.visibility !== 'hidden' && m.offsetWidth > 0) {
        visibleModals.push(m);
      }
    });
    
    if (anyModalOpen || visibleModals.length > 0) {
    ...
    // Check if we're on a tab that shows products (Tracker or similar)
    const activeTab = document.querySelector('[class*="tab"].active, [class*="Tab"].active, button.active, [aria-selected="true"]');
    const activeText = activeTab ? (activeTab.textContent || '').toLowerCase() : '';
    
    // Also look for product table/list
    const hasProducts = document.querySelectorAll('td, [class*="product-row"], [class*="ProductRow"]').length > 3;
    
    // Show search bar when we see product data
    if (hasProducts || (activeText.includes('track') || activeText.includes('product') || activeText.includes('all') || activeText.includes('inventory'))) {
      if (!wrap.classList.contains('visible')) {
        wrap.classList.add('visible');
        // Insert before the first table or product grid
        const firstTable = document.querySelector('table, [class*="grid"], [class*="list"], main > div > div');
        if (firstTable && firstTable.parentNode) {
          firstTable.parentNode.insertBefore(wrap, firstTable);
        }
        setTimeout(doProductSearch, 300);
      }
    } else {
      // Hide search bar on other tabs
      if (wrap.classList.contains('visible')) {
        wrap.classList.remove('visible');
        clearSearchHighlights();
      }
  ...
  function doProductSearch() {
    const input = document.getElementById('ai-product-search-input');
    const count = document.getElementById('ai-search-count');
    if (!input) return;

    const query = input.value.trim().toLowerCase();
    
    // Find all product rows/cards
    ...
    for (const row of rows) {
      ...
      // Skip header rows, empty rows, and non-product rows
      if (!text || text.includes('product') && text.split(' ').length < 3) {
        if (row.tagName === 'TR' && row.querySelector('th')) continue;
      }

      const isActionRow = row.querySelector('button, a, input, select');
      if (!isActionRow && row.children.length <= 1) continue;
      
      totalCount++;

      if (!query) {
        row.classList.remove('ai-search-hidden', 'ai-search-highlight');
        visibleCount++;
      } else if (text.includes(query)) {
        row.classList.remove('ai-search-hidden');
        row.classList.add('ai-search-highlight');
        visibleCount++;
      } else {
        row.classList.add('ai-search-hidden');
        row.classList.remove('ai-search-highlight');
      }
    }

    // Also search in div-based product cards
    const cards = document.querySelectorAll('[class*="product"], [class*="Product"], [class*="card"], [class*="Card"]');
    for (const card of cards) {
      if (card.id === 'ai-product-search-wrap') continue;
      const text = (card.textContent || '').toLowerCase();
      if (!text || text.length < 3) continue;
      
      if (card.closest('.ai-search-hidden')) continue;
      
      if (!totalCount && card.children.length >= 2) {
        totalCount++;
        if (!query) {
          card.classList.remove('ai-search-hidden');
          visibleCount++;
        } else if (text.includes(query)) {
          card.classList.remove('ai-search-hidden');
          visibleCount++;
        } else {
          card.classList.add('ai-search-hidden');
        }
      }
    }
    ...
    if (count) {
      if (query) {
        count.innerHTML = '<strong>' + visibleCount + '</strong> of <strong>' + totalCount + '</strong> products match "<strong>' + query + '</strong>"';
      } else {
        count.innerHTML = '<strong>' + totalCount + '</strong> products total';
      }
    }

    // Remove highlight after 2 seconds
    clearTimeout(window.aiHighlightTimeout);
    window.aiHighlightTimeout = setTimeout(function() {
      document.querySelectorAll('.ai-search-highlight').forEach(function(el) {
  ...
  function clearSearchHighlights() {
    document.querySelectorAll('.ai-search-hidden, .ai-search-highlight').forEach(function(el) {
      el.classList.remove('ai-search-hidden', 'ai-search-highlight');
    });
    const count = document.getElementById('ai-search-count');
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #ai-product-search-wrap.visible { display: block; }
      #ai-product-search-wrap input {
        width: 100%;
      ...
      #ai-product-search-wrap input:focus {
        border-color: #764ba2;
        box-shadow: 0 0 0 3px rgba(102, 126, 234, 0.2);
      }
      #ai-product-search-wrap .search-count {
        font-size: 12px;
        color: #888;
        margin-top: 6px;
        text-align: right;
      }
      #ai-product-search-wrap .search-count strong {
        color: #667eea;
      }
  ...
  function watchForProductForms() {
    // Aggressively watch for ANY dialog/modal/overlay appearing
    const observer = new MutationObserver(function() {
      // Look for ANY visible dialog, modal, or overlay
      const dialogs = document.querySelectorAll('[role="dialog"], [class*="modal"], [class*="dialog"], [class*="overlay"], [class*="popup"], [class*="drawer"]');
      for (const dlg of dialogs) {
        if (!dlg.dataset.aiWatched) {
          dlg.dataset.aiWatched = 'true';
          // Wait for form to render
          setTimeout(function() {
            attachToDialogInputs(dlg);
          }, 300);
        }
      }
      // Also check any newly visible inputs
      attachToVisibleInputs();
    });
    observer.observe(document.body, { childList: true, subtree: true });
    
    // Also poll frequently for newly opened modals
    setInterval(attachToVisibleInputs, 1500);
  }

  function attachToDialogInputs(container) {
    // Find ALL text inputs in the dialog
    var inputs = container.querySelectorAll('input[type="text"], input:not([type]), input[type="search"]');
    for (var i = 0; i < inputs.length; i++) {
      var inp = inputs[i];
      if (inp.id === 'ai-scanner-input' || inp.id === 'ai-chat-input' || inp.id === 'ai-product-search-input' || inp.dataset.aiEnhanced) continue;
      
      // Check if this is likely a product name field
      if (isProductNameInput(inp)) {
        inp.dataset.aiEnhanced = 'true';
        setupAutoCategorize(inp);
      }
    }
  }
  ...
  function isProductNameInput(input) {
    ...
    if (prev) prevLabel = (prev.textContent || '').toLowerCase();
    
    // Check if it's in a dialog/modal (likely a form field)
    var inDialog = !!input.closest('[role="dialog"], .modal, [class*="dialog"], [class*="overlay"]');
    
    // Keywords that suggest this is a product name field
    var nameKeywords = ['name', 'product', 'item', 'title', 'description', 'search product', 'enter product'];
    var matched = false;
    for (var k = 0; k < nameKeywords.length; k++) {
      if (ph.includes(nameKeywords[k]) || ariaLabel.includes(nameKeywords[k]) || 
          prevLabel.includes(nameKeywords[k]) || parentText.includes(nameKeywords[k])) {
        matched = true;
        break;
      }
    }
    
    // If it's in a dialog and has no obvious non-name attributes, it might be the name field
    if (inDialog && !input.type === 'password' && !input.type === 'email' && !input.type === 'number' && 
        !ph.includes('password') && !ph.includes('email') && !ph.includes('search')) {
      // If it's the first or second text input in the dialog, it's likely the name
      if (matched) return true;
      
      var dialog = input.closest('[role="dialog"], .modal, [class*="dialog"], [class*="overlay"]');
      if (dialog) {
        var textInputs = dialog.querySelectorAll('input[type="text"], input:not([type])');
        // If it's the 1st or 2nd text input, it's very likely the name field
        if (textInputs.length > 0 && (textInputs[0] === input || textInputs[1] === input)) {
          // Skip if placeholder indicates it's something else
          if (!ph.includes('password') && !ph.includes('email') && !ph.includes('price') && 
              !ph.includes('cost') && !ph.includes('barcode') && !ph.includes('sku')) return true;
        }
      }
    }
  ...
  function findNameInput() {
    // First try: look in any open dialog/modal
    var dialogs = document.querySelectorAll('[role="dialog"], .modal, [class*="dialog"], [class*="overlay"]');
    for (var d = 0; d < dialogs.length; d++) {
      var inputs = dialogs[d].querySelectorAll('input[type="text"], input:not([type])');
      for (var i = 0; i < inputs.length; i++) {
        if (inputs[i].id === 'ai-scanner-input') continue;
        if (isProductNameInput(inputs[i])) return inputs[i];
      }
    }
    
    // Second try: look anywhere on the page
    var inputs = document.querySelectorAll('input[type="text"], input:not([type])');
    for (var i = 0; i < inputs.length; i++) {
      if (inputs[i].id === 'ai-scanner-input' || inputs[i].id === 'ai-chat-input' || inputs[i].id === 'ai-product-search-input') continue;
      if (isProductNameInput(inputs[i])) return inputs[i];
    }
  ...
  function setupAutoCategorize(nameInput) {
    ...
    nameInput.addEventListener('input', function() {
      ...
      var existing = document.getElementById('ai-cat-suggestion');
      if (existing) existing.remove();
      var val = nameInput.value.trim();
      if (val.length < 2) return;
      debounceTimer = setTimeout(function() { autoCategorize(val, nameInput); }, 400);
    });
    setTimeout(function() { if (findCategoryInput(nameInput)) nameInput.dataset.categoryInput = 'true'; }, 500);
  }

  function findCategoryInput(nameInput) {
    var form = nameInput.closest('form') || nameInput.closest('[role="dialog"]') || nameInput.closest('.modal') || nameInput.closest('div[class*="form"]');
    if (form) {
      ...
      for (var i = 0; i < selects.length; i++) {
        ...
        if (sel.options && sel.options.length > 2) {
          ...
          if (opts.some(function(o) { return o === 'other' || o.includes('category') || o.includes('dairy') || o.includes('meat') || o.includes('beverage'); })) return sel;
        }
      }
      ...
      for (var i = 0; i < inputs.length; i++) {
        if (inputs[i] !== nameInput) {
          ...
          if (ph.includes('category') || ph.includes('type')) return inputs[i];
        }
      }
    }
    return null;
  ...
  function autoCategorize(name, nameInput) {
    ...
    if (name.length >= 2) {
      fetch('/api/ai/suggest?q=' + encodeURIComponent(name), { headers: { 'Authorization': 'Bearer ' + token } })
        .then(function(r) { return r.json(); })
        .then(function(data) { if (data && data.length > 0) showNameSuggestions(nameInput, data); })
        .catch(function() {});
    }
  }
  ...
  function showCategorySuggestion(nameInput, category, confidence) {
    setTimeout(function() {
      var categoryInput = findCategoryInput(nameInput);
      if (categoryInput) {
        ...
        setTimeout(function() { categoryInput.style.background = ''; }, 2000);
      }
    }, 200);
  }

  function showNameSuggestions(nameInput, suggestions) {
    var existing = document.getElementById('ai-name-suggestions');
    if (existing) existing.remove();
    ...
    dropdown.innerHTML = suggestions.map(function(s) {
      return '<div class="ai-cat-suggestion-item" data-name="' + s.name.replace(/"/g, '&quot;') + '" data-category="' + (s.category || 'Other').replace(/"/g, '&quot;') + '">' +
        '<span class="cat-name">' + s.name + '</span>' +
        '<span class="cat-label">' + (s.category || 'Other') + '</span></div>';
    }).join('');
    
    dropdown.addEventListener('click', function(e) {
      var item = e.target.closest('.ai-cat-suggestion-item');
      if (item) {
        nameInput.value = item.dataset.name;
        nameInput.dispatchEvent(new Event('input', { bubbles: true }));
        dropdown.remove();
        setTimeout(function() {
          var catInput = findCategoryInput(nameInput);
          if (catInput && item.dataset.category && item.dataset.category !== 'Other') {
            if (catInput.tagName === 'SELECT') {
              for (var i = 0; i < catInput.options.length; i++) {
  ...
  function watchForNameInputs() {
    // Already handled by watchForProductForms above
  }
  ...
  function addQuickSuggestions(panel) {
    var sugs = panel.querySelector('#ai-quick-sugs');
    if (!sugs) return;
    sugs.innerHTML = [
      ...
      return '<button onclick="var i=document.getElementById(\'ai-chat-input\');var b=document.getElementById(\'ai-chat-send\');if(i){i.value=\'' + s.replace(/'/g, "\\'") + '\';if(b)b.click()}">' + s + '</button>';
    }).join('');
