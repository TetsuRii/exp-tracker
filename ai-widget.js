...
function addSearchBar() {
  ...
  // Don't append to body yet - wait for checkTab to insert it in the right place
  // Just keep it in memory until we find the right container
...
function checkTab() {
  ...
  // Check for things that are DEFINITELY modals (ones that block interaction)
  var defModal = document.querySelectorAll(
  ...
  for (var i = 0; i < defModal.length; i++) {
    if (defModal[i].offsetWidth > 0) { modalShowing = true; break; }
  }

  // 🛑 Only hide if a real modal is showing
  if (modalShowing) {
    if (w.classList.contains('vis')) { w.classList.remove('vis'); clearSrch(); }
    return;
  }

  // Check if we're on ANY tab that has a product table visible
  var hasTable = document.querySelectorAll('table').length > 0;
  var hasRows = document.querySelectorAll('td').length > 3;
  var hasProdClass = document.querySelectorAll('[class*="product"]').length > 5;

  // Also check if this is the tracker tab by looking at ANY active tab
  var active = document.querySelector('[class*="tab"].active, [aria-selected="true"], [class*="Tab"].active, button.active, a.active');
  var atxt = active ? (active.textContent || '').toLowerCase() : '';

  // Show if we see table data OR are on a relevant tab
  var shouldShow = (hasTable && hasRows) || hasProdClass || 
                   atxt.indexOf('track') > -1 || atxt.indexOf('product') > -1 || 
                   atxt.indexOf('inventory') > -1 || atxt.indexOf('all') > -1;

  // Also show on ANY tab that has table rows (even if empty header)
  if (!shouldShow && hasTable) {
    var tds = document.querySelectorAll('td');
    shouldShow = tds.length > 1 || document.querySelectorAll('th').length > 1;
  }
  ...
  if (shouldShow) {
    if (!w.classList.contains('vis')) {
      ...
      var target = document.querySelector('table, [class*="grid"], [class*="list"], main > div, [class*="content"], [class*="table"], .MuiTableContainer-root');
      if (target && target.parentNode) {
        target.parentNode.insertBefore(w, target);
      } else {
        // If no table found, insert it at the top of the main content area
        var main = document.querySelector('main, #root > div, [class*="layout"]');
        if (main) main.insertBefore(w, main.firstChild);
      }
...
function addSuggester() {
  // Watch for new inputs appearing (especially in modals)
  var mo = new MutationObserver(function() {
    findAndEnhance();
  });
  ...
  // Run immediately
  findAndEnhance();
  // Run repeatedly as React renders components over time
  var retries = 0;
  var iv = setInterval(function() {
    findAndEnhance();
    retries++;
    if (retries > 30) clearInterval(iv); // Stop after 30 seconds
  }, 1000);
}
...
function findAndEnhance() {
  ...
  for (var i = 0; i < inputs.length; i++) {
    var inp = inputs[i];
    // Skip our own inputs
    if (inp.id === 'ai-si' || inp.id === 'ai-srch-i' || (inp.id && inp.id.indexOf('ai-') === 0)) continue;
    if (inp.dataset.aiEnh) continue;

    var ph = (inp.placeholder || '').toLowerCase();
    var aria = (inp.getAttribute('aria-label') || '').toLowerCase();

    // Is this a product name input? Be VERY generous
    var isName = ph.indexOf('name') > -1 || ph.indexOf('product') > -1 || ph.indexOf('item') > -1 || 
                 aria.indexOf('name') > -1 || aria.indexOf('product') > -1;

    // If it's inside any dialog/modal, it's likely a form field
    var inDlg = !!inp.closest('[role="dialog"], .modal, [class*="dialog"], [class*="overlay"], [class*="modal"]');

    // Exclude obvious non-name fields
    var notExcluded = ph.indexOf('password') === -1 && ph.indexOf('email') === -1 && 
                      ph.indexOf('barcode') === -1 && ph.indexOf('phone') === -1 &&
                      ph.indexOf('search') === -1 && ph.indexOf('filter') === -1;

    // Enhance if: it's clearly a name field, OR it's inside a dialog and not excluded
    if (isName || (inDlg && notExcluded)) {
      inp.dataset.aiEnh = '1';
      setupSuggest(inp);
    }
  }
