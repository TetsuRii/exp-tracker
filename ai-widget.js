...
function boot(){
  ...
  setInterval(doDates,7000)
  setTimeout(doDates,2000)
}
...
function css(){
  ...
  s.textContent=`
.ai_t{position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#1a1a2e;border:2px solid #4caf50;padding:12px 20px;z-index:999999;max-width:500px;width:90%;animation:af .3s;color:#fff;font-size:14px;display:flex;align-items:center;gap:12px}
...
// ==================== DATES ====================
// Only runs every 7 seconds. First removes ALL old badges, then adds fresh ones.
// This prevents badges from stacking up.

async function doDates(){
  var t=token()
  if(!t)return

  // Step 1: Remove ALL existing date badges from the entire page
  document.querySelectorAll('._db').forEach(function(e){e.remove()})
  // Also remove leftover <br> that might be orphaned before badges
  document.querySelectorAll('._dbbr').forEach(function(e){e.remove()})
  ...
  // Step 3: For each row, try to match a product
  ...
  for(var i=0;i<rows.length;i++){
    ...
    for(var p=0;p<prods.length;p++){
      ...
      if(txt.indexOf(pn)>-1&&pn.length>bestScore){
        bestScore=pn.length
        best=prod
      }
    }
    ...
    span.className='_db'
    var label=''
    if(best.updatedAt)label='✏️ Edited '+ago(best.updatedAt)
    else if(best.addedAt)label='➕ Added '+ago(best.addedAt)
    if(label){
